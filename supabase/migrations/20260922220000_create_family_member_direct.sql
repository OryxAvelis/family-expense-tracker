...
create or replace function public.create_darnaflow_member_direct(
  p_family_id uuid,
...
begin
  ...
  values (p_family_id, 'personal', v_member_id);

  return query select v_legacy_user_id;
end
$$;

revoke all on function public.create_darnaflow_member_direct(
  uuid, bigint, text, text, text, text
) from public, anon, authenticated;
grant execute on function public.create_darnaflow_member_direct(
  uuid, bigint, text, text, text, text
) to service_role;
